...
local multiplier = settings.global["pollution_detector_multiplier"].value
...
script.on_event(defines.events.on_runtime_mod_setting_changed, function(event)
  if not event then return end
  if event.setting == "pollution_detector_multiplier" then
    multiplier = settings.global["pollution_detector_multiplier"].value
  end
end)
